...
observations are independent of each other, and the populations have the same variance. Throughout the paper we assume a null hypothesis of zero for the population mean difference.
...
Due to these differences in the standard errors and degrees of freedom, Student's and Welch's t tests can disagree about whether the group means are different. Because Welch's t test decreases the degrees of freedom, it might be more conservative. If Student's t test finds 
...
\subsection{Visualizing Data with Boxplots} 
    ...
    A second way to decide which test to use is to visualize the data using boxplots and make a judgment 
...
simulations with equal variances or of simulations with unequal variances. 
...
didn't hurt. Figure 1 shows the Welch's t test degrees of freedom could 
drop below 70\% of Student's when only the variances 
ratio or only the sample sizes were unequal, but the simulated false positive rates, 

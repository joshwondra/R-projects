...
and effect sizes. We then expanded these simulations to tests of interactions in a 2 x 2 factorial design. We recommend the following rule: For normally distributed data always use Welch's t test to 
...
In the pooling equation (EQ 2) the variance of the group 
with the larger sample size is given more weight than the variance of the group with the 
...
the Bayesian analytic solution is identical to Student's t test. When the same priors are used, 
but equal variances are not assumed, the Bayesian solution is similar to Welch's t 
...
and seeing if the boxplots of your data look like the typical boxplots of the 
...
The use of t tests can be expanded to study designs with more than two groups, where they can be used to test main effects, interactions, or other specific contrasts between the groups. For these tests, the researcher again has the choice of Student's or Welch's t test. We tested the interaction of a 2 x 2 design using Student's and Welch's t tests both when there were no interaction or main effects (to test the type I error rate) and when there was a cross-over interaction but no main effect (to test power). When there was a true cross-over interaction, as shown in Figure 7, the difference in means between groups B and C vs. groups A and D was the same as the simulations of two groups with a medium-sized effect. We tested the interaction when all sample sizes and variances were equal, as well as under selected conditions when the sample size and variance of just one group (group A) differed from the others.

\subsubsection{Type I Error Rates}
As with the case of two groups, the type I error rate for Student's t test was near the expected $\alpha = .05$ rate when either the sample sizes or variances were equal (see Figure 8). But when both the sample sizes and variances were unequal, the type I error rate varied widely. If just one group had a larger sample than the others, the type I error rate was too high if it had the smaller variance, and too low if it had the larger variance; and if just one group had a smaller sample than the others, the type I error rate was too low if it had the smaller variance and too high if it had the larger variance. In contrast, the type I error rate for Welch's t test was consistently near .05.
...
\subsubsection{Power}
As with the case of two groups, the power of Student's t test to detect an effect was similar to the power of Welch's t test when either sample sizes or variances were equal (see Figure 9). But when both the sample sizes and variances were unequal, the power of Student's t test was highest under the conditions when it had the greatest risk of type I errors. In contrast, Welch's t test was more powerful than Student's t test under some conditions without having an increased risk of type I errors. 

<<interaction data power, echo = FALSE>>=
##### Student's t test data
...
## Put it in a dataframe
int.power.classic <- data.frame(
    power=c(midd.ve.ne, midd.v5.ne.bigvar, midd.v5.ne.smallvar, midd.ve.n2.bign, midd.ve.n2.smalln, midd.v5.n2.bignbigvar, midd.v5.n2.bignsmallvar, midd.v5.n2.smallnbigvar, midd.v5.n2.smallnsmallvar),
...
## Plot it
Pow.classic <- ggplot(int.power.classic, aes(x=as.numeric(var.ratio), y=power, ymin=0, ymax=.13)) + 
    geom_point(size=4, aes(shape = Ns, color = Ns)) + 
    scale_shape_manual(name = 'Sample size ratio \nGroup A,B,C,D',
                       labels = c('50,50,50,50', '100,50,50,50','50,100,100,100'),
                       values= rep(1, 3)
    ...
    scale_x_continuous(breaks=seq(-1:1), labels=factor(c('1/5', '1', '5'))) +
    scale_y_continuous(breaks=seq(from=0, to=1, by=.1)) +
    xlab(bquote(frac(sigma[A]^2,{sigma[B]^2 == sigma[C]^2} == sigma[D]^2))) + ylab('Power (Medium Effect)') +
    theme(panel.grid.minor.y = element_blank(), panel.grid.major.x = element_blank(), panel.grid.minor.x = element_blank())
...
## Plot it
Pow.welch <- ggplot(int.power.welch, aes(x=as.numeric(var.ratio), y=power, ymin=0, ymax=.13)) + 
    geom_point(size=4, aes(shape = Ns, color = Ns)) + 
    scale_shape_manual(name = 'Sample size ratio \nGroup A,B,C,D',
                       labels = c('50,50,50,50', '100,50,50,50','50,100,100,100'),
                       values= rep(1, 3)
...
##### Difference in power
int.power.diff <- int.power.classic
int.power.diff$power <- int.power.classic$power - int.power.welch$power

## Plot it
Pow.diff <- ggplot(int.power.diff, aes(x=as.numeric(var.ratio), y=power, ymin=0, ymax=.13)) + 
    geom_point(size=4, aes(shape = Ns, color = Ns)) + 
    scale_shape_manual(name = 'Sample size ratio \nGroup A,B,C,D',
                       labels = c('50,50,50,50', '100,50,50,50','50,100,100,100'),
                       values= rep(1, 3)
    ...
    xlab(bquote(frac(sigma[A]^2,{sigma[B]^2 == sigma[C]^2} == sigma[D]^2))) + ylab('Difference in Power') +
    theme(panel.grid.minor.y = element_blank(), panel.grid.major.x = element_blank(), panel.grid.minor.x = element_blank())

@

\begin{figure*}    
<<PowerInt, echo=FALSE, fig=TRUE, height=10>>=
multiplot(Pow.classic+ggtitle("Student's t test"), Pow.welch+ggtitle("Welch's t test"), Pow.diff+ggtitle("Student - Welch"))
@

\textit{Figure 9.} Power for Student's t test and Welch's t test 
of a 2 x 2 interaction as the variance and sample size of one group varies.
\end{figure*}


\subsubsection{Coverage Probability}
Complementing the type I error rate, the coverage probability for Student's t test was near the expected .95 rate when either the sample sizes or variances were equal (see Figure 10). But when both the sample sizes and variances were unequal, the coverage probability varied widely. If just one group had a larger sample than the others, the coverage probability was too low if it had the smaller variance, and too high if it had the larger variance; and if just one group had a smaller sample than the others, the coverage probability was too high if it had the smaller variance, and too low if it had the larger variance. In contrast, the coverage probability for Welch's t test was consistently near .95.


<<ClassicCoverageInt, echo=FALSE>>=

##### Plot classic results only

## Set up data
nullT.ve.ne <- int.obs.coverage.ve.ne[,1,1]
nullT.v5.ne.bigvar <- int.obs.coverage.v5.ne.bigvar[,1,1]
nullT.v5.ne.smallvar <- int.obs.coverage.v5.ne.smallvar[,1,1]
nullT.ve.n2.bign <- int.obs.coverage.ve.n2.bign[,1,1]
nullT.ve.n2.smalln <- int.obs.coverage.ve.n2.smalln[,1,1]
nullT.v5.n2.bignbigvar <- int.obs.coverage.v5.n2.bignbigvar[,1,1]
nullT.v5.n2.bignsmallvar <- int.obs.coverage.v5.n2.bignsmallvar[,1,1]
nullT.v5.n2.smallnbigvar <- int.obs.coverage.v5.n2.smallnbigvar[,1,1]
nullT.v5.n2.smallnsmallvar <- int.obs.coverage.v5.n2.smallnsmallvar[,1,1]


## Put it in a dataframe
coverage.classic.nullT <- data.frame(
    coverage.rate=c(nullT.ve.ne, nullT.v5.ne.bigvar, nullT.v5.ne.smallvar, nullT.ve.n2.bign, nullT.ve.n2.smalln, nullT.v5.n2.bignbigvar, nullT.v5.n2.bignsmallvar, nullT.v5.n2.smallnbigvar, nullT.v5.n2.smallnsmallvar), #NOTE: doubled any with equal variance or 
#equal ns because they don't have separate ssv and bsv versions
...
## Plot it
Cov.classic <- ggplot(coverage.classic.nullT, aes(x=as.numeric(var.ratio), y=coverage.rate, ymin=.85, ymax=1)) + 
    geom_point(size=4, aes(shape = Ns, color = Ns)) + 
    scale_shape_manual(name = 'Sample size ratio \nGroup A,B,C,D',
                       labels = c('50,50,50,50', '100,50,50,50','50,100,100,100'),
                       values= rep(1, 3)
    ...
    scale_x_continuous(breaks=seq(-1:1), labels=factor(c('1/5', '1', '5'))) +
    scale_y_continuous(breaks=seq(from=.85, to=1, by=.01)) +
    xlab(bquote(frac(sigma[A]^2,{sigma[B]^2 == sigma[C]^2} == sigma[D]^2))) + ylab('Coverage Probability') +
    theme(panel.grid.minor.y = element_blank(), panel.grid.major.x = element_blank(), panel.grid.minor.x = element_blank())

@


<<WelchCoverageInt, echo=FALSE>>=

##### Plot classic results only

## Set up data
nullT.ve.ne <- int.obs.coverage.ve.ne[,1,2]
nullT.v5.ne.bigvar <- int.obs.coverage.v5.ne.bigvar[,1,2]
nullT.v5.ne.smallvar <- int.obs.coverage.v5.ne.smallvar[,1,2]
nullT.ve.n2.bign <- int.obs.coverage.ve.n2.bign[,1,2]
nullT.ve.n2.smalln <- int.obs.coverage.ve.n2.smalln[,1,2]
nullT.v5.n2.bignbigvar <- int.obs.coverage.v5.n2.bignbigvar[,1,2]
nullT.v5.n2.bignsmallvar <- int.obs.coverage.v5.n2.bignsmallvar[,1,2]
nullT.v5.n2.smallnbigvar <- int.obs.coverage.v5.n2.smallnbigvar[,1,2]
nullT.v5.n2.smallnsmallvar <- int.obs.coverage.v5.n2.smallnsmallvar[,1,2]
...
## Plot it
Cov.welch <- ggplot(coverage.welch.nullT, aes(x=as.numeric(var.ratio), y=coverage.rate, ymin=.85, ymax=1)) + 
    geom_point(size=4, aes(shape = Ns, color = Ns)) + 
    scale_shape_manual(name = 'Sample size ratio \nGroup A,B,C,D',
                       labels = c('50,50,50,50', '100,50,50,50','50,100,100,100'),
                       values= rep(1, 3)
...
\begin{figure*}
<<CoveragePlotsInt, echo=FALSE, fig=TRUE>>=
layout <- matrix(c(1,2), nrow=2, byrow=TRUE)
multiplot(Cov.classic + ggtitle("Student's t test"), Cov.welch + ggtitle("Welch's t test"), layout=layout)
@

\textit{Figure 10.} Coverage probabilities for Student's and Welch's t tests of a 2 x 2 interaction as the variance and sample size of one group varies.
\end{figure*}
...
rule is to always use Welch's t test to compare the means of independent groups, whether it is a contrast between just two groups or a contrast between more than two groups.
...
larger variance. This can be seen in Figures 3, 6, 8, and 10, where the type I error 
...
The simulations show that the pattern is not straightforward and depends on complex combinations between which group has the larger sample size and which group has the larger variance.  In the case of between-groups contrasts, we arrived at a simple conclusion: on balance, when considering the false positive rate and power, an efficient strategy is to always uses the Welch t test. As the study design becomes more complicated, such as with repeated measures, random effect models, complex contrasts, non-normally distributed data, etc., the story will likely be even more complicated. As a field we should move toward an overall analysis strategy that has relatively few moving parts so that it can be described easily in a scientific report, is relatively easy to describe in a  preregistration plan, and provides robust estimates.

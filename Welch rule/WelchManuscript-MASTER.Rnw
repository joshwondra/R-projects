...
difference:   
    \begin{equation}
    t = \frac{\mu_1-\mu_2}{\sigma_{\mu_1-\mu_2}}
    \end{equation}
...
common variance to compute the standard error: 
    \begin{equation}
    \sigma_{\mu_1-\mu_2} = \sqrt{\frac{\sigma^2}{n}} = \sqrt{\frac{\sigma_1^2}{n}} = \sqrt{\frac{\sigma_2^2}{n}}
    \end{equation}
...
the sample variance of one of the groups for the common variance 
to find the standard error. Student's t test deals with this 
...
variances are different from each other. It can also produce degrees of freedom with decimals.
...
Our goal is to find a simple decision rule that can help researchers decide when to use Welch's t test instead of Student's t test.
...
group means. Our purpose in this paper is to inform decisions about the use of 
...
If the test retains the null hypothesis, then the variances are equal and you can use Student's t test. 

One example is Levene's test for homogeneity \cite{Levene1960}, which shows up in SPSS by default when you run an independent samples t test.
...
when the raw data are not normally distrubuted \cite{Brown1974}.
...
when the ratio falls below 96\% in sample data to allow for some variation from sampling error.
...
variances are unequal, there were differences in power. Overall, Student's t 
...
t test didn't hurt. Figures 1 shows that the Welch's t test degrees of freedom could 
...
negligible effect on the outcomes. We illustrate this point in Figure 7, which shows two 
...
larger than the tails of the distribution with the greater degrees of freedom, but 
...
that effect size estimation can also involves assumptions about group variances.
    ...
    If the effect size of interest is a confidence interval for the difference in
group means, you must make a decision about whether or not to assume that the
...
highly unstable estimates. Under some conditions, the confidence intervals were 
...
weight to the larger sample. If the larger sample has the larger variance, the 
...
because there is no common standard deviation, so there can be no true Cohen's d.
...
has a few advantages. First, unlike Cohen's d, reporting raw descriptive statistics does not require the researcher to commit to an 
...
Welch's t test as a safe first step that will do just fine without imposing 

...
The use of t tests can be expanded to study designs with more than two groups, where they can be used to test main effects, interactions, or other specific contrasts between the groups. For these tests, the researcher again has the choice of Student's or Welch's t test. We tested the interaction of a 2 x 2 design using Student's and Welch's t tests when the means of the four groups were equal and when there was a cross-over interaction, but no main effects (Figure 7). The difference in means between groups B and C vs. groups A and D is the same as the difference for a medium-sized effect from simulations with two groups. We tested the interaction contrast when all sample sizes and variances were equal, as well as under selected conditions when the sample size and variance of just one group (group A) differed from the others.

\subsubsection{Type I Error Rates}
As with the case of two groups, the type I error rate for Student's t test was near the expected $\alpha = .05$ rate when either the sample sizes or variances were equal. But when both the sample sizes and variances were unequal, the type I error rate varied widely. If just one group had a larger sample than the others, the type I error rate was too high if it had the smaller variance and too low if it had the larger variance; and if just one group had a smaller sample than the others, the type I error rate was too low if it had the smaller variance and too high if it had the larger variance. In contrast, the type I error rate for Welch's t test was consistently near .05.

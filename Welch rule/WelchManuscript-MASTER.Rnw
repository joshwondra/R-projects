...
multiplot <- function(..., plotlist=NULL, file, cols=1, layout=NULL, title='') {
 ...
 if (numPlots==1) {
    ...
    # Set up the page
    grid.newpage()
    pushViewport(viewport(layout = grid.layout(1+nrow(layout), ncol(layout), heights=unit(c(.5,rep(1,nrow(layout))),'null'))))
    ...
    for (i in 1:numPlots) {
      ...
      print(plots[[i]], vp = viewport(layout.pos.row = matchidx$row+1, #adjusted this to make sure the first plot doesn't print on the title row
                                      layout.pos.col = matchidx$col))
    }
  }
 grid.text(paste(title), vp=viewport(layout.pos.row=1,layout.pos.col=1:ncol(layout))) # I added this part for a title 
}
...
ve.distributions <- qplot(x=x, y=y, fill=group, colour=group, geom='line') + geom_vline(xintercept=as.numeric(by(x,group,mean)))

set.seed(2184)
ve.bplots <- c(list(ve.distributions), lapply(1:4, function(x){qplot(x=factor(rep(c(1,2), each=10)), y=c(rnorm(10,6,2),rnorm(10,7.13,2)), geom='boxplot', colour=factor(rep(c(1,2), each=10)), xlab='', ylab='') + theme(legend.position='none')}), lapply(1:4, function(x){qplot(x=factor(rep(c(1,2), each=100)), y=c(rnorm(100,6,2),rnorm(100,7.13,2)), geom='boxplot', colour=factor(rep(c(1,2), each=100)), xlab='', ylab='') + theme(legend.position='none')}))

layout <- matrix(c(0,1,1,0,2,3,4,5,6,7,8,9), nrow=3, byrow=TRUE)
multiplot(plotlist=ve.bplots, layout=layout, title='Figure 1. Boxplots for Equal Variances When Sample Sizes Equal 10 and 100')
...
set.seed(2184)
ve.bplots <- c(list(ve.distributions), lapply(1:4, function(x){qplot(x=factor(rep(c(1,2), each=10)), y=c(rnorm(10,6,2),rnorm(10,7.13,10)), geom='boxplot', colour=factor(rep(c(1,2), each=10)), xlab='', ylab='') + theme(legend.position='none')}), lapply(1:4, function(x){qplot(x=factor(rep(c(1,2), each=100)), y=c(rnorm(100,6,2),rnorm(100,7.13,10)), geom='boxplot', colour=factor(rep(c(1,2), each=100)), xlab='', ylab='') + theme(legend.position='none')}))

layout <- matrix(c(0,1,1,0,2,3,4,5,6,7,8,9), nrow=3, byrow=TRUE)
multiplot(plotlist=ve.bplots, layout=layout, title='Figure 2. Boxplots for Unequal Variances When Sample Sizes Equal 10 and 100')
...
ggplot(data.frame(x=seq(2,10,.1)), aes(x)) +
    ...
    labs(title='Figure 3. Degrees of Freedom Ratio When Sample Sizes are Equal and Variances are Unequal', y=bquote(frac('df'['Welch'],'df'['Student'])), x=bquote(frac('s'^1,'s'^2))) +
...
ggplot(data.frame(x=seq(50,100,.1)), aes(x)) + 
    ...
    labs(title='Figure 4. Degrees of Freedom Ratio When Sample Sizes are Unequal and Variances are Equal', y=bquote(frac('df'['Welch'],'df'['Student'])), x=bquote(frac('n'^1,'n'^2))) +
...
ggplot(data.frame(x=seq(2,10,.1)), aes(x)) + 
    ...
    labs(title='Figure 5. Degrees of Freedom Ratio When Sample Sizes are Unequal and Variances are Unequal', y=bquote(frac('df'['Welch'],'df'['Student'])), x=bquote(frac('s'^1,'s'^2))) +
    scale_x_continuous(breaks=c(2,4,6,8,10), labels=c(2,4,6,8,10)/2)

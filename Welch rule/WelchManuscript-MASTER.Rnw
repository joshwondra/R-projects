...
\subsection{Comparing the Degrees of Freedom of the Two Tests}
    ...
    A different way to decide which t test test might be to compare the
...
are unequal. For example, perhaps it is typical for the Welch's t degrees of freedom 
to be 95-100\% of the Student's t degrees of freedom when variances are equal, but 
when the Welch's t degrees of freedom are 90\% of the Student's t degrees of freedom 
or less, then that reliably indicates that the population variances are unequal. 

    We examined whether the ratio of the degrees of freedom from Welch's t 
test to the degrees of freedom from Student's t test could provide a heuristic for deciding that 
the group variances are unequal. To our knowledge, this possibility has never
been examined. We used the analytical formulas for the two 
degrees of freedom to see how the degrees of freedom (df) ratio should change as 
the population variances and the sample sizes of the two groups change.

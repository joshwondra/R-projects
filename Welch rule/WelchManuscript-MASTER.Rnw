...
difference:   
    ...
    \end{equation}
Throughout the pape we assume a null hypothesis of zero for the population mean difference.    The p-value also depends on the degrees of 
...
as the t-value and 
df get larger---when 
the sample size gets larger (which increases the df and influences the standard error), when the 
...
simulations with equal variances. 
    ...
    Figure 1 displays five sample boxplots from groups with equal 
...
100 (second rows). 
  ...
  When the 
sample sizes are smaller it's typical to see 
...
\textit{Figure 1.} Five sample boxplots for groups with equal variances (top) and unequal 

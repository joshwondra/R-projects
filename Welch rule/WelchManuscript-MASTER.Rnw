...
considered to represent smaller effects when variances are unequal.
    ...
    For each condition (i.e., combination of variance ratio,smaller sample size and sample size ratio), we set the seed to of the random number generator and ran 10,000 simulations. 

...
rn_plot <- function(data, sim, which_contrast, which_test) {
  ...
  ylims <- if (sim == 'No effects') {
    c(0, .15)
    } else if (sim == 'Crossover interaction' & str_detect(which_contrast, 'Main')) {
      c(0, .15)
    } else if (sim == 'Crossover interaction' & str_detect(which_contrast, 'Int')) {
      c(0, .15)
    } else if (sim == 'Main effects & interaction') {
      c(0, 1)
    }
  
  ybreaks <- if (sim == 'No effects') {
    seq(0, 1, .05)
    } else if (sim == 'Crossover interaction' & str_detect(which_contrast, 'Main')) {
      seq(0, 1, .05)
    } else if (sim == 'Crossover interaction' & str_detect(which_contrast, 'Int')) {
...
When Student's t test is used, the group variances are still estimated from a common pooled variance, and information from all four groups is used to find the pooled variance. 

\begin{equation}
    s_p^2 = \frac{\sum (n_{i} - 1)s_{i}^2}{\sum (n_{i} - 1)}
\end{equation}

To compute the standard error, you take each group's squared contrast weight multiplied by its estimated variance and divided by its sample size, sum them, and find the square root of the sum. For each group the estimated variance is the pooled variance because we assume variances are equal. 
...
However, the benefit of using a t test is that you don't \textit{have} to use Student's t test---you can use Welch's t test instead. The value of the contrast is the same, but its standard error is based on separate variances rather than a common variance. Just like Student's t test, you take each group's squared contrast weight multiplied by its estimated variance and divided by its sample size, sum them, and find the square root of the sum. But unlike Student's t test, for each group the estimated variance is that group's sample variance because we do not assume variances are equal. 
...
\begin{figure}[!ht]
<<no_effects_coverage_probability, echo = FALSE, collapse = TRUE, fig = TRUE, width = 8, height = 4>>=

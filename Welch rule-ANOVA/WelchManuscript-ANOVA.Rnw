...
powdiff_plot <- function(data, sim, which_contrast) {
  ...
  data %>% 
    ...
    mutate(
      power_diff = ANOVA - `Welch's t test`,
      test = "ANOVA - Welch's"
      ) %>% 
...
crossover_pow <- plot_grid(
  ...
  blank_plot,
  blank_plot,
  rn_crossover_int_welch + theme(strip.background = element_blank()),
  
  # third row
  blank_plot,
  blank_plot,
  powdiff_crossover_int + theme(strip.background = element_blank()),
...
me_int_pow <- plot_grid(
  ...
  # second row
  rn_me_int_meA_welch + theme(strip.background = element_blank(), strip.text.y = element_blank()), 
  rn_me_int_meB_welch + theme(strip.background = element_blank(), strip.text.y = element_blank()),
  rn_me_int_int_welch + theme(strip.background = element_blank()),
  
  # third row
  powdiff_me_int_meA + theme(strip.background = element_blank(), strip.text.y = element_blank()), 
  powdiff_me_int_meB + theme(strip.background = element_blank(), strip.text.y = element_blank()),
  powdiff_me_int_int + theme(strip.background = element_blank()),
...
\textit{Figure 4.}Power for the simulations when there was a crossover interaction or two main effects and an interaction.
\end{figure}

We can compare the power of ANOVA and Welch's t test to detect the main effects and the interaction effect, and in the case of a significant interaction effect, we can compare their power to detect the simple effects of factor B at each level of factor A. 

...
The degrees of freedom are $df = \sum (n_i - 1)$, and the t value is equal to the square root of the F value.
...
With no effects, we can compare the traditional ANOVA/Student's t test approach to Welch's t test on the false positive rate for each comparison (two main effects and one interaction), the joint false positive rate for the three comparisons, and effect size estimation. 
...
variance, the false positive rate 
rose as high as .26.
This means that if a researcher were to run four 2~$\times$~2 experiments in a year that had no true effects, but they analyzed them with ANOVA, we would expect one experiment to show at least one false positve.   
...
difference in group means (which is 0 when there are no effects). By 
...
current_data <- no_effects %>% 
  ungroup() %>% 
  select(-joint_reject_student, -joint_reject_welch, -reject_student, -reject_welch) %>% 
...
current_data %>% 
  ...
  facet_grid(contrast_names ~ test) +
  scale_x_discrete(labels = factor(c('1/5', '1/2', '1', '2', '5'))) +
  scale_y_continuous(breaks = seq(.8, 1, .01)) +
  labs(
    x = bquote(frac(sigma[A1/B1]^2,{sigma[A1/B2]^2 == sigma[A2/B1]^2} == sigma[A2/B2]^2)),

...
\subsection{Similarities and Differences Between Student's and Welch's t tests with Two Groups}
...
where $\sigma_1^2$ and $\sigma_2^2$ are the population variances of the two groups. 

  But Student's t test and Welch's t tests make different assumptions about how to estimate the group variances. 
  Student's t test assumes the population variances of the groups are equal, so if the sample variances are different, it's due to sampling error. 
...
average
    ...
    \begin{equation}
    s_{\overline{x}_1-\overline{x}_2} = \sqrt{\frac{s_p^2}{n_1}  + \frac{s_p^2}{n_2}} %= s_p\sqrt{\frac{1}{n_1}  + \frac{1}{n_2}} Josh W cut this because it's easier to see how the standard error from Student's and Welch's t tests compare by leaving the pooled variance under the radical
    \end{equation}
...
freedom. Student's t test uses a simple formula to compute the degrees of freedom: $df=n_1+n_2-2$. Welch's t test uses a more complicated formula to compute the degrees of freedom: 
    \begin{equation}
...
\citeA{Delacre2017} argued that researchers should use Welch's t test by default because it has better control over the false positive rate when the group variances are unequal, but it doesn't lose much power when the variances are equal. 
...
Each contrast is the sum of the weights multiplied by their respective group means.
\begin{equation}
...
When Student's t test is used, the standard error is still estimated from the common variance among the groups, and information from all the groups is used to find the pooled variance. Then the pooled variance is weighted by both the group sample sizes and contrast weights to compute the standard error. 
\begin{equation}
    s_p^2 = \frac{\sum (n_{i} - 1)s_{i}^2}{\sum (n_{i} - 1)}
\end{equation}
\begin{equation}
    s_{\hat{I}} = \sqrt{\sum \frac{a_i s_p^2}{n_i}}
\end{equation}

The degrees of freedom are $df = \sum (n_i - 1)$, and the t value is equal to the square root of the F value from an ANOVA with Type 3 Sum of Squares, or any Type Sum of Squares if the group sample sizes are equal.

However, the benefit of expressing the test as a t test is that you don't \textit{have} to use Student's t test---you can use Welch's t test instead. The value of the contrast is the same, but its standard error is based on separate variances rather than a common variance.
\begin{equation}
    s_{\hat{I}} = \sqrt{\sum \frac{a_i s_{i}^2}{n_{i}}}
\end{equation}

The degrees of freedom also generalize to use information from all the groups.
\begin{equation}
    df = \frac{(\sum \frac{s_i^2}{n_i})^2}{\sum \frac{(s_i^2)^2}{n_i^2(n_i-1)}}
\end{equation}
...
<<display_effects, echo = FALSE, collapse = TRUE, fig = TRUE, width = 6, height = 2>>=
...
ggplot(display_data, aes(y = y, x = factorA, group = factorB, linetype = factorB)) +
  geom_line() +
  labs(x = 'Factor A', group = 'Factor B', linetype = 'Factor B', y = NULL) +
  facet_wrap(~ effect) +
  theme_classic() +
  theme(
...
\textit{Figure 1.} Means for the simulations when there were no effects, a crossover interaction with no main effects, or main effects and an interaction.
\end{figure}

\section{Case 1. No effects in a 2 x 2 factorial design}

In the case of no effects in a 2~$\times$~2 factorial design, we will compare the traditional ANOVA approach to Welch's t test on the false positive rate for each comparison (two main effects and one interaction), the joint false positive rate for the three comparisons, and effect size estimation. 
...
about .02, but when it had the smaller 
...
<<no_effects_false_positives, echo = FALSE, collapse = TRUE, fig = TRUE, width = 6, height = 5>>=

# subset data
current_data <- no_effects %>% 
  ungroup() %>% 
  ...
  mutate(
    ...
    contrast_names = factor(contrast_names, levels = c('ME (1 & 2 vs 3 & 4)', 'ME (1 & 3 vs 2 & 4)', 'Interaction'), labels = c('Main Effect of A', 'Main Effect of B', 'Interaction')),
...
current_data %>% 
  ...
  labs(
    x = bquote(frac(sigma[A1/B1]^2,{sigma[A1/B2]^2 == sigma[A2/B1]^2} == sigma[A2/B2]^2)),
...
\textit{Figure 2.} False positive rates for ANOVA and Welch's t test
...
---the probability that at least one of the three comparisons produces a false positive. 
...
When the larger group(s) had the larger variance, the false positive rate dropped as low as 
about .07, but when it had the smaller 
variance, the false positive rate 
rose as high as .26. The observed joint false positive  
rate for Welch's t test 
was more variable than it was for the individual comparisons, but it remained closer to the expected rate. Welch's 
...
current_data <- no_effects %>% 
  ...
  mutate(
    test = case_when(
      test == 'joint_reject_student' ~ "ANOVA",
...
\textit{Figure 3.} Joint false positive rates for ANOVA and Welch's t test
as the variance ratio, sample size ratio, and sample sizes vary.
\end{figure}
...
We can compare the power of ANOVA and Welch's t test to detect the interaction effect, but also to detect the two simple effects of factor B at each level of factor A. Figure 5 displays the difference in power between Welch's t test and ANOVA to detect the true effects, with higher values indicating that Welch's is more powerful. As we can see, the power of the two tests to detect the interaction was approximately equal when either the sample sizes or the variances were equal. However, Welch's t test had more power when the small group had the smaller variance. ANOVA had more power when the small group had the larger variance, but this was the same condition when it was more likely to find false positives. 

There were some important differences in the patterns for the simple effects. First was the magnitude of the difference, which was up to 11\% for the interaction, but up to 32\% for the simple effects. 

Second was the effect of unequal variances when the sample sizes were equal. In the previous analyses, when the sample sizes were equal, the two tests were similar, even if the variances were unequal. However, for the simple effects, when the sample sizes were equal, the two tests performed differently when the variances were unequal.

Third was the pattern of the difference in power when the group with unequal sample sizes and variances was included vs. excluded from the simple effect. The middle graph displays the contrast that includes the group that had different sample sizes or variances. The pattern was similar to the interaction effect---Welch'st test had more power when the small group had the smaller variance, and ANOVA had more power when the small group had the larger variance. The bottom graph displays the contrast that \textit{excludes} the group that had different sample sizes or variances. In this case, the pattern of the differences was opposite in direction and the difference in magnitude was asymmetrical. The ANOVA had more power to detect an effect when the small group had the smaller variance, but the difference was small-about 4-6\% when one group had 1/5 the variance of the other groups. However, Welch's t test had more power to detect an effect when the small group had the larger variance, and the difference could be quite large, up to 32\%.
...
current_data <- crossover_sims %>% 
  ...
  mutate( 
    contrast_names = factor(contrast_names, levels = c('Interaction', 'SE (1 vs 2)', 'SE (3 vs 4)'), labels = c('Interaction', 'Simple Effect (A1/B1 vs. A1/B2)', 'Simple Effect (A2/B1 vs. A2/B2)'))
    )

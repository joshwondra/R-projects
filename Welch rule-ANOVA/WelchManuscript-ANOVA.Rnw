...
First, past research has focused on research designs with two groups; we show how Welch's t test can be used to test any one degree of freedom contrast in designs with more than two groups.
Second, the justification for using Welch's t test rests heavily on emphasizing control of false positives and minimizing concerns with loss of power; we show that this is unnecessary, because Welch's t test is more powerful than Student's in common situations with no serious downsides.
Third, no attention has been paid to estimates of effect size; we show that 
Welch's t test does a better job estimating effect sizes regardless of the false positive rates or power of the two tests. 
}
...
for(i in 1:length(which_test)) {
  ...
  this_plot = joint_reject_data %>% 
    ...
    ggplot(aes(y = get(plot_name), x = var_ratio, group = ns, linetype = ns, shape = ns)) +
...
First, prior research has focused on research designs with only two groups. Although this is the prototypical case where psychologists would think to use a t test, they are also used in designs with more than two groups. Less commonly, in factorial designs with two levels per factor, t tests can be used instead of ANOVA to test the main effects and interactions. More commonly, in oneway or factorial designs with any number of levels per factor, t tests are used for contrasts between specific pairs of groups, or specific combinations of groups, to test the specific hypotheses that psychologists typically care about more than the omnibus ANOVA. We illustrate both with a 2~$\times$~2 factorial design.

Second, the main justification for using Welch's t test has been that it does a better job controlling false positives, and that the potential loss of power is minimal. However, researchers typically hope to see significant effects, and so a loss of power, however small, may discourage them from choosing Welch's t test. A stronger justification would more thoroughly address this concern with the loss of power. We show that Welch's t test can be much more powerful than Student's t test without serious downsides, and that when Student's t test is significantly more powerful it comes with big risks. 

Third, prior research has focused on null hypothesis reject rates, and little attention has been paid to estimates of effect size. This is a different and important topic to consider, because while one test may be more powerful than the other, it may also do a worse job at estimating the true effect size. Effect size estimation is related to false positive rates, but deserves discussion on its own. We show that Welch's t test does a better job estimating the correct effect size, regardless of false positive rates or power.

Through Monte Carlo simulations of a 2~$\times$~2 factorial design, we show that Welch's t test expands to cases with more than two groups, does a better job controlling false posities than a typical analysis of variance (ANOVA)/Student's t test, can be more powerful with little downside, and can more accurately estimates effect sizes, so that Welch's t test should be used by default to test any one degree of freedom contrast in a variety of research designs.
...
variance, then the standard error will be larger; if the larger group has the smaller variance, then the standard error will be smaller.
  ...
  In contrast Welch's t test does not assume the group variances are equal. The sample variances of the groups are used to estimates their population variances, and they are used directly to estimate the standard error fo the t test:
    \begin{equation}
...
Our opening discussion focused on the case of two groups, but Welch's t test can generalize to any between-groups design (for a discussion in the context of one-way ANOVA, see Delacre et al., 2019)
...
\begin{equation}
    s_{\hat{I}_{Welch}} = \sqrt{\sum a_i\frac{s_{i}^2}{n_{i}}}
\end{equation}

The degrees of freedom also generalize to use information from all the groups.


NOTE: Need to correct this here and in the simulations - forgot to add the contrast weights, which will have a small impact on the simple effects
\begin{equation}
...
Welch's t test by default \cite{Zimmerman1996,Moser1992,Moser1989}, and expand this recommendation to designs with more than two groups.
For researchers who 
insist on using ANOVA by default, they must come up with a decision rule for when they will switch to Welch's t test, or some other test that doesn't assume variances are equal. One way to decide which test to use
is to run another test of the null hypothesis that the group variances 
are equal. If the test retains the null, the variances are equal and you can use ANOVA test. If the test rejects the null, the variances are unequal and you should use Welch's t test.

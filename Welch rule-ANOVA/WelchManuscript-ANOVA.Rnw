...
When Student's t test is used, the standard error is still estimated from the common variance among the groups, and information from all four groups is used to find the pooled variance. 
\begin{equation}
    s_p^2 = \frac{\sum (n_{i} - 1)s_{i}^2}{\sum (n_{i} - 1)}
\end{equation}
\begin{equation}
    s_{\hat{I}} = \sqrt{\sum \frac{a_i^2 s_p^2}{n_i}}
\end{equation}
...
However, the benefit of using a t test is that you don't \textit{have} to use Student's t test---you can use Welch's t test instead. The value of the contrast is the same, but its standard error is based on separate variances rather than a common variance.
\begin{equation}
...
\textit{Figure 1.} Means for the simulations when there were no effects, a crossover interaction with no main effects, or two main effects and an interaction.
\end{figure}

\section{Case 1. No effects in a 2 x 2 factorial design}

In the case of no effects in a 2~$\times$~2 factorial design, we will compare the traditional ANOVA/Student's t test approach to Welch's t test on the false positive rate for each comparison (two main effects and one interaction), the joint false positive rate for the three comparisons, and effect size estimation. 
...
in a 2~$\times$~2 factorial design (factors A and B)
...
---the probability that at least one of the main effects or the interaction produces a false positive. 
...
We can compare the power of ANOVA and Welch's t test to detect the interaction effect, but also to detect the two simple effects of factor B at each level of factor A. Figure 5 displays the difference in power between Welch's t test and ANOVA to detect the true effects, with higher values indicating that Welch's is more powerful. 
...
And for the contrast between grouops A2B1 and A2B2, groups A2B1 and A2B2 get weights of 1 and the other groups get weights of 0

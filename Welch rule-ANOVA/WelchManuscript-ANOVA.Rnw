...
The t tests are constructed using the same formulas as those that we use for main effects and interactions, and the only difference is the contrast weights.
...
Although such contrasts are commonly run as follow-up tests to an interaction, we support running them first if they are specific tests of the key hypothesis. (NOTE: Let's save this for the discussion)

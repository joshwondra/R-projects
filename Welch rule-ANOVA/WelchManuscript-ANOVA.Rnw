...
We'll illustrate with three examples:
\begin{itemize}
\item no main effects, no interaction
\item no main effects, crossover interaction
\item two main effects, interaction
\end{itemize}

For each example, we ran 10,000 simulations of four independent groups with normally distributed data.
We looked at cases where the variances and sample sizes were equal or where the variance and/or sample size of just one group differed from the other three groups.
...
t test consistently behaved as expected when it came to false positives, whether there were no effects or an interaction with no main effects. 
ANOVA did not.

...
Recently, some researchers have recommended that psychologists should use Welch's t test as the default.
While these recommendations have focused on the case of two groups or a one-way ANOVA, we demonstrate that Welch's t test can generalize to any between-groups design, and illustrate with the case of a 2~$\times$~2 factorial design. 
We used Monte Carlo 
simulations to compare the false positive rate, power, and coverage probability 
of classic ANOVA and Welch's t tests across different variance ratios, sample size ratios, sample sizes, 
and effects. We recommend that
...
In this paper, we compare contrasts based on Welch's t-test to a typical analysis of variance (ANOVA). We show that in designs with more than two groups, Welch's t test does a better job controlling false positives, accurately estimating effect sizes, and simplifying pre-registration, without losing too much power. We illustrate with a 2~$\times$~2 factorial design.
...
different variance ratios and sample size ratios (FOOTNOTE: We also looked at different population effect sizes and other choices for sample sizes, but they had no impact on the relative effects of ANOVA and Welch's t test, so we do not show them here (NOTE: should give some sense of the ranges of the effect sizes and sample sizes examined)).
...
(NOTE: fig 1 needs to label the standard and welch plots)
\begin{figure}[!ht]  
...
(NOTE Fig 2 can have narrower rangeo on the y axis to highlight the patterns, like 5\% to 20\%)
\begin{figure}[!ht]  

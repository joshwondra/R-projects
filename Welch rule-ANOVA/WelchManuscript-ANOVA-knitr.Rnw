...
average
    ...
    \end{equation}
This common variance is used to estimate the population variances of the two groups ($\hat{\sigma}_1^2 = \hat{\sigma}_2^2 = s_p^2$), which can then be used to estimate the standard error of the t test.  
    \begin{equation}
...
Our opening discussion focused on the case of two groups, but Welch's t test can generalize to any between-groups design.
...
Aside from the main effects and interaction, researchers often have hypotheses about differences between specific groups.
These differences can be tested with Welch's t test by changing the contrast weights.  
For example, here are the contrast weights to test the simple effects of A at each level of B, and of B at each level of A:
...
These focused contrasts are often run as follow-up tests to a significant test of main effects or interactions, 
but we support running them first if they are the key tests of the researcher's hypotheses.
If the main effects and interactions don't address the researchers's hypotheses, then they're uninformative.
If they're used to decide whether you have permission to run more focused contrasts (e.g., only run the contrasts if you see a significant main effect), then they unnecessarily decrease power to detect true effects. 
In designs with more than two conditions per factor, it's less likely that the researcher is predicting general main effects or interactions, and these points become more important, 
and although there is a Welch version of ANOVA that tests for differences between more than three groups \cite{Delacre2019, Welch1951}, we'd encourage researchers to stick with the focused contrasts that directly test their hypotheses. 
...
We'll illustrate with three examples: no effects, a crossover interaction with no main effects, and two main effects with an interaction (see Figure 1). 
For each example, we ran 10,000 simulations of four independent groups with normally distributed data.
We looked at cases where the variances and sample sizes were equal or where the variance and/or sample size of just one group ($A_1 B_1$) differed from the other three groups.
We compared how well ANOVA and Welch's t test balance concerns with 
false positives, power, and effect size estimation under 
different variance ratios and sample size ratios.\footnote{We also looked at different effect sizes and sample sizes, but they had no meaningful impact on the relative benefits of ANOVA vs. Welch's t test, so we do not show them here.} We tested the main effects, the interaction effect, and focused contrasts for the simple effects of A at each level of B, and for the simple effects of B at each level of A.  

\begin{figure}[!ht]  
<<three_examples, echo = FALSE, fig.width=12, fig.height=4>>=

plot_grid(
    no_effects_true_effect_plot, 
    crossover_true_effect_plot, 
    me_int_true_effect_plot,
    ncol = 3
  )
@

\caption{Examples used to test ANOVA vs. Welch's t test.}
\end{figure}
...
To understand how well ANOVA and Welch's t test control false positives, we looked at tests of the main effects and interaction when there were no effects, and at tests of the main effects when there was only a crossover interaction. 
...
\begin{figure}[!ht]  
<<false_positives, echo = FALSE, warning = FALSE, fig.width=14, fig.height=10>>=
...
\caption{False positives for the main effects and interaction.}
\end{figure}


\subsection{Joint False Positives for Main Effects and Interaction}

In addition to looking at each test separately, we can look at the joint false positive rate for the main effects and interaction
...
\begin{figure}[!ht]  
\centering
<<no_effects_joint_false_positives, echo = FALSE, collapse = TRUE, fig.width=8, fig.height=8, out.width = '50%'>>=
data_plots <- plot_grid(
...
\caption{Rates of at least one false positive (at least one main effect or the interaction) for the simulations when there were no effects.}
\end{figure}

For ANOVA, the observed joint false positive
rate was close to the expected rate when either 
the population variances or sample sizes were equal, but it varied widely 
...
\subsection{Focused Contrasts for the Simple Effects}


In addition to looking at main effects and interactions, we can look at the false positive rates for the simple effects of A at each level of B, and the simple effects of B at each level of A. 
To understand the false positive rates, we looked at all of the simple effects when there were no effects, and at the simple effect of B at $A_2$ when there was a main effect and interaction (because the main effect of B was completely driven by the simple effect of B at $A_1$).
...
But unlike the main effects and interactions, the false positive rate for ANOVA varied whenever the variance of one group was different from the other groups, even when the sample sizes were equal (see Figure 4).

\begin{figure}[!ht]
<<se_false_positives, echo = FALSE, warning = FALSE, fig.width=16, fig.height=10>>=
...
@
\caption{False positives for the simple effects.}
\end{figure}
...
When group $A_1 B_1$ was included in the contrasts, if its variance was smaller than the other groups, ANOVA was too conservative, and if its variance was larger than the other groups, ANOVA was too liberal. 
But when group $A_1 B_1$ was excluded from the contrasts, the pattern was opposite (though the size of the effect was smaller)--if its variance was smaller, ANOVA was too liberal, and if its variance was larger, ANOVA was too conservative. 
...
\begin{equation}
\begin{split}
    s_p^2 & = \frac{\sum (n - 1)s_{i}^2}{\sum (n - 1)} \\
    & = \frac{(n - 1)(s_{A_1 B_1}^2 + s_{A_1 B_2}^2 + s_{A_2 B_1}^2 + s_{A_2 B_2}^2)}{4(n - 1)} \\
    & = \frac{1}{4}(s_{A_1 B_1}^2 + s_{A_1 B_2}^2 + s_{A_2 B_1}^2 + s_{A_2 B_2}^2)
\end{split}
\end{equation}
...
So when ANOVA is used and the sample sizes are equal, then even if a group is excluded from the contrast, its variance is weighted just as much as the other groups to estimate the standard error. However, when Welch's t test is used and the sample sizes are equal, then only the groups that are \textit{included} in the contrast are used to estimate the standard error.
...
Comparing equations 15 and 16, we can see that even though the sample sizes are equal, ANOVA is giving too much weight to the variances from groups that are excluded from the contrasts, and not enough weight to the variances from the groups that are included in the contrasts.\footnote{Note that if all variances are equal, Equations 15 and 16 are equal.}
So when the groups that are excluded from the contrast have the smaller variance, the standard error from ANOVA is too small, which will inflate the false positive rate, and when they have the larger variance, the standard error from Student's t test is too large, which will decrease the false positive rate.
...
\begin{figure}[!ht]
<<power_plots, echo = FALSE, collapse = TRUE, fig.width=12, fig.height=8>>=
...
\begin{figure}[!ht]
<<coverage_probability, echo = FALSE, collapse = TRUE, fig.width=12, fig.height=8>>=
...
\begin{figure}[!ht]
<<se_power_plots, echo = FALSE, collapse = TRUE, fig.width=12, fig.height=8>>=
...
\begin{figure}[!ht]
<<se_coverage_probability, echo = FALSE, collapse = TRUE, fig.width=12, fig.height=8>>=
...
\begin{figure}[!ht]
<<boxplots, echo = FALSE, fig.height=8>>=
